...
class WassExperimentRunner:
            ...
            for cluster_size in self.config.cluster_sizes:
                for rep in range(self.config.repetitions):
                    ...
                    for method in self.config.scheduling_methods:
                        ...
                        result = self._run_single_simulation(workflow, cluster, method)
    ...
    def _generate_scenario(self, task_count: int, cluster_size: int, seed: int):
        ...
        for i in range(task_count):
            for j in range(i + 1, task_count):
                if np.random.rand() < 0.2: # 20%的依赖概率
                    tasks[j]["dependencies"].append(tasks[i]["id"])

        workflow = {"tasks": tasks}
        return workflow, cluster

    def _run_single_simulation(self, workflow: Dict, cluster: Dict, method: str) -> ExperimentResult:
        """运行一次完整的、决策驱动的仿真"""
        ...
        while pending_tasks:
            ready_tasks = []
            for task_id in sorted(list(pending_tasks)): # 按ID排序以确保确定性
            ...
            for task in ready_tasks:
                ...
                # --- 执行与状态更新 ---
                task_flops = task['flops'] / 1e9 # GFlops
                node_cpu = cluster[chosen_node]['cpu_capacity'] # GFlops/s
        ...
        for task in workflow['tasks']:
            for dep in task['dependencies']:
                if task_placements[task['id']] != task_placements[dep]:
                    transfers += 1
        total_deps = sum(len(t['dependencies']) for t in workflow['tasks'])
            ...
            def make_decision(self, state):
                ...
                for node, est in state.cluster_state['earliest_start_times'].items():
                    ...
                    if finish_time < earliest_finish_time:
                        ...
                        best_node = node
                return SchedulingAction(state.current_task, best_node, 1.0)
    ...
    def _get_heft_scheduler(self, workflow, cluster):
        ...
        class HeftScheduler:
            ...
            def _calculate_ranks(self, workflow, cluster):
                ...
                avg_exec_time = {tid: np.mean([(t['flops']/1e9)/n['cpu_capacity'] for n in cluster.values()]) for tid, t in tasks.items()}
                ...
                def get_rank(task_id):
                    ...
                    max_succ_rank = 0
                    if successors:
                        max_succ_rank = max(get_rank(succ_id) for succ_id in successors)
                    
                    ranks[task_id] = avg_exec_time[task_id] + max_succ_rank
            ...
            def make_decision(self, state):
                # HEFT的核心是选择能让当前最高rank任务最早完成的节点
                if state.current_task != self.task_order[self.next_task_idx]:
                     # HEFT有固定顺序，如果当前任务不是预期的，就跳过（在我们的循环中这不会发生）
                     # 但为了鲁棒性，我们还是选择一个默认节点
                     return SchedulingAction(state.current_task, state.available_nodes[0], 1.0)
    ...
    def _save_and_analyze_results(self):
        ...
        if fifo_results:
            baseline_makespan = np.mean([r.makespan for r in fifo_results])
        ...
        for method in self.config.scheduling_methods:
            ...
            improvement = ((baseline_makespan - avg_makespan) / baseline_makespan) * 100 if baseline_makespan else 0
