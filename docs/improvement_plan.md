...
**目标**: 在现有 HEFT 与 MIN-MIN 对比中引入可控扰动，放大两种策略在 Makespan 上的差异，为后续 RAG 消融和基线对比提供更具辨识度的数据。

**方向 1：极端平台异构性**

- 调整或新增平台 XML，使部分主机的计算速率显著低于其他主机，同时引入非对称带宽/延迟设置，迫使调度策略在资源选择上产生明显差异。
- 在 `configs/platform_*.xml` 引入至少一个“慢速+瓶颈链路”主机组合，并更新 `configs/workflow_config.yaml` 以便快速切换。
- 目标指标：HEFT 倾向避开慢速节点，MIN-MIN 可能持续分配从而造成 Makespan 至少扩大 10%。

**方向 2：数据本地性与沟通惩罚**

- 在 `src/simulation/schedulers.py` 与 `src/simulation/experiment_runner.py` 内加入可选的输入输出数据位置模型，为跨主机调度添加显式通信成本。
- 如果 WRENCH 文件定位仍报错，先实现“逻辑延迟”版本：对非本地主机执行的任务附加额外时间，模拟数据移动。
- 目标指标：HEFT 的全局排序可减少跨节点迁移次数，而一次性贪心的 MIN-MIN 受到更大通信惩罚，从而拉大差距。

**方向 3：结构化合成工作流**

- 在 `scripts/augment_workflows.py` 或生成器脚本中扩展 DAG 模板，引入“迷惑性分支”：长链关键路径 + 多个低成本支路同时出现，且其任务规模在不同层次呈现反差。
- 通过参数化方式控制关键路径的连贯性和其他支路的任务粒度，确保 HEFT 优先守护关键路径，而 MIN-MIN 容易被支路吸引导致关键链延迟。
- 目标指标：关键路径 Makespan 在 MIN-MIN 下延长 ≥15%，而 HEFT 基本保持原值。

**执行建议**

- 每次改动后运行 `python scripts/4_run_experiments.py --strategies HEFT MINMIN --seeds 0-4`，收集 `results/synthetic_smoke` 中的均值与标准差。
- 记录配置参数（平台文件名、生成器超参、通信惩罚系数），便于后续论文撰写与复现实验。
