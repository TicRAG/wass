...
class PerformancePredictor(nn.Module):
    """Minimal MLP predictor with .predict API expected by WASSRAGScheduler.

    If instantiated without explicit input_dim (legacy usage), user must call
    load_state_dict with a matching shape before prediction.
    """
    def __init__(self, input_dim: int = 64, hidden_dim: int = 128, model_type: str = "MLP", model_path: Optional[str] = None):
        super().__init__()
